...
- 8-direction animation wired up on the shared `PlayerAnimation2D` controller: each blend tree (`Idle`/`Walk`/`Run`) now has the four diagonal children at the exact `Snap8` positions `(±0.71, ±0.71)`, and `Player - ZQSD` ships with `DirectionMode = Snap8`. The diagonals currently reuse the left/right clips as placeholders — swap in dedicated diagonal art later without touching the positions.
### Why
...
- Renamed the `Player2D` prefab to `Player - ZQSD` (GUID preserved) and aligned it with the other 2D player prefabs: root scale ×8 and a `CapsuleCollider2D` matching `Player - ClickToMove` (replacing the old `CircleCollider2D`). It stays a pure `Rigidbody2D` WASD/ZQSD mover (no `NavMeshAgent`, no `Visual` child — the feet-on-root split is a NavMesh-only workaround it does not need).
### Removed
