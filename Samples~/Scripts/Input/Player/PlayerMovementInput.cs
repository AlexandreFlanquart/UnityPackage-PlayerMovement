...
namespace MyUnityPackage.ControllerSample
{
    public class PlayerMovementInput : MonoBehaviour, IPlayerMovement, PlayerControls.IPlayerMovementActions
    {
        ...
        public void OnClick(InputAction.CallbackContext context)
        {
            ...
            // Pointer.current covers Mouse, Touchscreen and Pen (works in "Input System only" projects).
            Pointer pointer = Pointer.current;
            if (pointer == null)
            {
                MUPLogger.Warning("Click performed but no pointer device (mouse/touch/pen) is available.");
                return;
            }
